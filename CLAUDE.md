...
Read [MVP Requirements](MVP%20Requirements) first. It defines what the first pilot must do, what shows "coming soon" and the launch gate. Every PR links one assigned TRAK issue and names the journey (J1–J7), guarantee (G1–G7), or scope/launch gate it serves. Parked feature work waits. Track progress in [the pilot journey index](docs/use-cases/PILOT-INDEX.md), and follow [the release gate](docs/release/merge-gate.md). [The September 25 readiness plan](docs/pilot-readiness-2026-09-25.md) is the 18 September task matrix. `docs/features-outstanding.md` is historical, not current readiness evidence. Read the actual route and callers before fixing a component; `CoachQuickMatchLog` is currently unrouted.

No issue, no code: agents work their human's assigned Linear issues. Include `TRAK-# [J/G]` (or `[scope]` for scope work) in the branch/PR naming, following the existing agent branch prefix where required, and never work on main. Keep one PR attached to one issue and record blockers as Linear dependencies. Workflow: Todo → In Progress → In Review → Verifying after merge/deploy → Done only after Makis's FINAL OK. Announce migrations and shared-file changes in #coding-agents-at-work before editing. Imad coordinates merges and posts the current merge queue there. A PR is approved by a verified review with a verdict (MERGE / MERGE AFTER / DO NOT MERGE) from someone who did not author it — see *What approval means* in the release gate. Commit only green; never bypass the pre-commit hook. Never apply development SQL to the shared live Supabase project. Use a disposable local database and new migrations; production deployment follows reviewed merges.

Friday 25 September is a **synthetic demo**. Real children join only when the launch gate in MVP Requirements passes and a founder majority agrees. A date never admits anyone, including `pilot_config.starts_on`.
