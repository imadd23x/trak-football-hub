...
- The rule when multiple guardians disagree or one withdraws is **pending**. Do not implement “any approval wins,” “latest approval wins” or “any withdrawal wins” without a decision.
...
- A separate versioned state/projection supports effective authorization and locking per `(child, academy)`. Keep per-guardian decisions distinct; the pending guardian-combination rule determines the projection. Repeated requests use an idempotency key without discarding distinct later decisions.
...
- Obtain the missing inputs below; agree guardian withdrawal semantics before implementing effective authorization. Reserve a new migration timestamp and shared interfaces with Kostas/Tarek.
...
- Add scoped status and withdrawal controls in `src/pages/Settings.tsx` or a focused parent consent screen. Show the selected academy and consequences before withdrawal; a failed request must not claim success. Multiple-guardian wording depends on the pending decision.
...
| Withdrawal | Selected academy loses authorization immediately after committed withdrawal; other academy state stays intact. Guardian-combination cases remain release-blocked until their expected outcomes are approved. |
...
- Multi-guardian approval/withdrawal precedence, whose parent visibility each choice authorizes, and dispute/reapproval handling.
