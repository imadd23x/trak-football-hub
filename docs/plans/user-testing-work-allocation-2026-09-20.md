...
- Next independent I-A recovery defect: a built-app probe on `c0999f9` held the initial `/auth/v1/user` response for 21 seconds. The screen still read “Checking your account…” with **zero buttons**; releasing that one response immediately produced “Continue as Alex Parent”. The probe had no unexpected requests or browser exceptions. `AuthContext.fetchOrCreateProfile` awaits `createOnboardingSession` without a deadline, and the latter's Auth lookup has no AbortSignal. This is a verified existing loading/recovery gap, not a failure of the new reset-email controls or a fix already delivered. Next work must bound initial/profile retry hydration, abort stale reads, preserve already usable profiles and idempotent provisioning, and test real provider/browser failure and account-switch behavior. Local evidence: `/tmp/trak-profile-hydration-probe.log`; a registered regression and fix remain to be added in their own branch.
