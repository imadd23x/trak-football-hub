...
| **fork #4** `d306355`, draft | Incremental UT-22 remembered-account choice on #74 `c910829`: 16 focused/436 source/17 harness tests, type/build and 15 browser journeys pass, including current-head fork CI. Tarek accepted the navigation logic at `b763063`; his unreachable-catch finding is corrected. Confirmation callbacks, household/consent and final-head review remain open. |
...
| #76 `252b486`, open | Kostas's focused UT-12 validation follow-up. Reviewed on native PostgreSQL: standalone four suites pass; composed 77 migrations expose two fixture conflicts with #44/#74. **Hold:** the form still sends attacker goals as `2+`, dropping goal credit; reuse #51 mapping/tests. [Review and reproducer](../reviews/pr76-current-head-2026-09-20.md). No merge approval. |
...
| #34 `7e8e9b5`, #36 `5bc730c`, #37 `93ae9ea` | One forward academy-history repair plus corrected registry declarations now appears in all three branches. Previous implementation heads passed both CI runs; current six-header follow-ups have scoped local verification and fresh CI running. All migration/SQL-body bytes are preserved. Keep the existing upgrade modes/hooks when combining #42. Trigger review and production approval remain. |
