...
| #42 `918d8c3`; #66 `404e729` | Tarek owns calendar contracts and the SQL suite registry; #38 dependency is satisfied. #66 carries an older snapshot of #42 work, not its current head (verified ancestry); it supplies scorecard tests. The latest `404e729` published/unpublished fixture correction is author-reported and awaits independent retest. Integrate #42 first, then reconcile #66 against that exact base. Do not overwrite current runner/birthday coverage with an older file. Verify each final delta after main changes. |
...
- #66 advanced to `404e729`; Tarek reports an explicitly published positive fixture and unpublished decoy, 22 assertions. That newer claim is not an independent retest yet. The earlier `de85bea` failure remains historical evidence, not a claim that the newest head still fails.
