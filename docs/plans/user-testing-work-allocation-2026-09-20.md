...
| #51 `5beba3b` | Tarek's exact goal-count/rating-key work overlaps `CoachAddSession` in #44. Reuse `match-input-keys` and its tests. It already stores exact numbers independently of rating buckets. The remaining range change is its ceiling/options (the earlier plan said 0–10; #76 proposes 0–20 and relational constraints, pending one agreed contract), plus boundary/consumer validation; do not redesign storage. Kostas owns the final form; Tarek hands off and reviews the mapping. |
