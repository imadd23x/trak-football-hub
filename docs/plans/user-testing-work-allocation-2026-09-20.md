...
- Next independent I-A defect is now reproduced separately: with an expired synthetic remembered session, hold the SDK's `/auth/v1/token` refresh for 21 seconds on built fork #8. Account choice remains “Checking your account…” with zero buttons and zero profile reads. Releasing that response immediately produces “Continue as Alex Parent”; no unexpected request or browser exception occurred. Thus the wait is before profile hydration starts. The diagnostic ran only against intercepted synthetic traffic; its successful execution confirms an unresolved defect, not successful recovery. Temporary reproducer: `/var/folders/jm/gmh1cp8s4p76h3flnjcww1g80000gn/T/trak-sdk-restore-probe-i8s2tfz3/e2e/sdk-restore.probe.ts`. A registered regression and scoped SDK lifecycle fix are still required; do not present a UI timeout as cancellation of the SDK's pending session mutation.
