...
September 20, 2026. Incremental fix on staff admission PR74 `c910829a088c0debaaf15c61ad897b1a77f00f84`; canonical main `4335e89` is an ancestor. This branch changes only the landing-page consumer, focused tests and the browser test registration. It does not redefine household admission or edit a peer-owned contract.
...
- Built-app Chromium: **14/14 journeys pass**, including all nine existing parent/staff journeys and five new account-choice cases. New coverage exercises 320/390 px, reload, keyboard Continue, failed logout then retry, failed password then successful switch to another family, an actual SDK-parsed email fragment and missing-profile recovery. Synthetic requests are intercepted through the real Supabase SDK. Unexpected requests and page exceptions are asserted absent.
- One initial browser assertion expected `/` exactly; the SDK correctly cleared the fragment token but retained an empty `#`. The test now requires the same origin/root path and an empty parsed hash. No application code was changed to satisfy that cosmetic expectation.
