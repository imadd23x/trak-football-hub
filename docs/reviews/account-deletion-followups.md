...
The upstream Settings confirmation promises that deletion permanently deletes all the user's data. P6 PR32 (`badb590`) replaces that claim with disclosure that some academy history and consent records may be retained; it is not merged or deployed at this review. That copy correction does not decide retention policy or change database/Storage behavior. P8 and U10 remain open until those decisions, implementation and deployed verification are complete.
