...
Before the separate reporting-view repair, the security advisor correctly remained **red**: 12 pre-existing owner-privilege reporting views and two unpinned pure functions were reported. With `20260918070209_restrict_pilot_operational_views.sql` integrated, the 60-migration PostgreSQL 17 replay passes all suites and the unchanged advisor returns **No issues found**, exit 0. This is local evidence, not a production advisor result. The native runner's initial Node PATH failure has its own passing regression test. Every temporary cluster was stopped and removed.
