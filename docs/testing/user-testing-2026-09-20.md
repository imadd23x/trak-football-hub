...
| UT-19 | Duplicate email signup falsely claims account/email/guardian invite created, same or different role | Reproduced and implemented on `parent/P5-signup-outcomes`; peer review/release pending, Imad | Nine real-SDK regressions and three mobile signup journeys pass; distinguish accepted request from delivery, sign-in/recovery, resend failure/retry, no unauthenticated provisioning. Real inbox delivery remains UT-21 |
