...
test('new staff activation through the actual Auth email fragment chooses a password and reaches coach home', async ({ page, context }, info) => {
  ...
  await page.getByRole('button', { name: 'Show new password' }).focus();
  ...
  await expect(page.getByLabel('New password', { exact: true })).toHaveValue('StrongStaff1!');
  expect(state.observed.filter(row => row.path === '/auth/v1/user' && row.body?.password)).toHaveLength(0);
  expect(state.observed.filter(row => row.path.endsWith('/accept_staff_invite'))).toHaveLength(0);
