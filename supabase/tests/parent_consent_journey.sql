...
BEGIN
  ...
  SELECT public.player_age_years(pg_temp.cj(20)) INTO v_age;
...
DO $test$
BEGIN
  PERFORM pg_temp.cj_assert(public.squad_player_consent_required(current_setting('trak.cj_sp')::uuid) IS TRUE,
...
BEGIN
  PERFORM pg_temp.cj_assert(public.squad_player_consent_required(v_sp) IS FALSE,
