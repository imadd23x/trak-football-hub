...
test('pinned main inventory precedes history, and unexpected historical changes fail closed', async () => {
  ...
  try {
    ...
    assert.equal(mainFiles.length, 66);
    ...
    assert.deepEqual(order.slice(0, 66), expectedMain);
    assert.ok(order.indexOf(reports) < order.indexOf(parent));
    assert.equal(order[66], historyMigration);
    assert.equal(order.length, 67);
...
test('actual CLI replays main first and executes all four role suites', { timeout: 65_000 }, async () => {
  const dir = await fixture();
  try {
    const result = await execute(dir);
    assert.equal(result.code, 0, result.output);
    assert.match(result.output, /Upgrade boundary: 66 migrations from main dc5c9d5682963fbff73f6117fe766c665db26776 applied; history RPCs absent/);
    assert.match(result.output, /Replayed 67 migrations \(exact main dc5c9d5 first, then pending parent history\)/);
