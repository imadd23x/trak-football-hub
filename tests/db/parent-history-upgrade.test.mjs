...
async function fixture() {
  ...
  for (const name of ['scripts/test-db.mjs', 'scripts/lib/parent-history-upgrade.mjs', 'scripts/migration-input.mjs']) {
...
test('pinned candidate inventory precedes history, and unexpected historical changes fail closed', async () => {
  ...
  try {
    ...
    assert.equal(mainFiles.length, 68);
    ...
    assert.deepEqual(order.slice(0, 68), expectedMain);
    assert.ok(order.indexOf(reports) < order.indexOf(parent));
    assert.equal(order[68], historyMigration);
    assert.equal(order.length, 69);
...
test('actual CLI replays candidate first and executes all six role suites', { timeout: 65_000 }, async () => {
  const dir = await fixture();
  try {
    const result = await execute(dir);
    assert.equal(result.code, 0, result.output);
    assert.match(result.output, /Upgrade boundary: 68 migrations from candidate 74a4a35e6d2633eff35cc6bbf71cf6e6b5a7e482 applied; history RPCs absent/);
    assert.match(result.output, /Replayed 69 migrations \(exact candidate 74a4a35 first, then pending parent history\)/);
    ...
    assert.match(result.output, /Privilege\/consent assertions: 151/);
    assert.match(result.output, /Passed: privilege_and_consent_security.sql/);
    assert.match(result.output, /Staff assertions: 51/);
    assert.match(result.output, /Staff delivery assertions: 27/);
  } finally { await rm(dir, { recursive: true, force: true }); }
});
...
test('actual CLI requires the newly merged privilege/consent suite completion report', { timeout: 65_000 }, async () => {
  const dir = await fixture();
  try {
    const file = resolve(dir, 'supabase/tests/privilege_and_consent_security.sql');
    const original = await readFile(file, 'utf8');
    assert.ok(original.includes('AS privilege_consent_assertions'));
    await writeFile(file, original.replace('AS privilege_consent_assertions', 'AS missing_privilege_report'));
    const result = await execute(dir);
    assert.equal(result.code, 1, result.output);
    assert.match(result.output, /Missing or incomplete privilege_and_consent_security.sql output: expected 151 assertions/);
    assert.doesNotMatch(result.output, /Passed: privilege_and_consent_security.sql/);
  } finally { await rm(dir, { recursive: true, force: true }); }
});

for (const [suite, key, count] of [
  ['staff_admission.sql', 'staff_assertions', 51],
  ['staff_delivery.sql', 'staff_delivery_assertions', 27],
]) test(`history upgrade cannot silently omit ${suite}`, { timeout: 65_000 }, async () => {
  const dir = await fixture();
  try {
    const file = resolve(dir, 'supabase/tests', suite);
    const original = await readFile(file, 'utf8');
    assert.ok(original.includes(`AS ${key}`));
    await writeFile(file, original.replace(`AS ${key}`, 'AS missing_staff_report'));
    const result = await execute(dir);
    assert.equal(result.code, 1, result.output);
    assert.ok(result.output.includes(`Missing or incomplete ${suite} output: expected ${count} assertions`));
    assert.ok(!result.output.includes(`Passed: ${suite}`));
  } finally { await rm(dir, { recursive: true, force: true }); }
});
