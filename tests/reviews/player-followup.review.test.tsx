/**
 * Opt-in desired-behavior audit of c9d99b8; no application replacements.
    ...
    server.use(
      ...
      http.get(endpoint('coach_calendar_events'), async ({ request }) => {
        ...
        return failSibling === 'coach_calendar_events'
          ? HttpResponse.json({ code: '42501', message: FAILURE }, { status: 403 })
          : project([{ id: 'synthetic-event', title: 'Synthetic upcoming training', event_type: 'training',
            starts_at: new Date(Date.now() + 86400000).toISOString(), published: true }], request)
      }),
